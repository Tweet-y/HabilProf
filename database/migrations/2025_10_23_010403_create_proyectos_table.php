...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('proyectos', function (Blueprint $table) {
            ...
            $table->foreign('rut_profesor_guia')->references('rut_profesor')->on('profesors')->onDelete('restrict'); 
            
            // FK de Rol de Profesor CO-GUIA (1:N) - NULL permitido (Opcional)
            $table->integer('rut_profesor_co_guia')->nullable(true);
            $table->foreign('rut_profesor_co_guia')->references('rut_profesor')->on('profesors')->onDelete('set null'); 
            
            // FK de Rol de Profesor COMISION (1:N) - NOT NULL
            $table->integer('rut_profesor_comision')->nullable(false);
            $table->foreign('rut_profesor_comision')->references('rut_profesor')->on('profesors')->onDelete('restrict');
