...
## F2: Ingreso de Datos a la Habilitación Profesional (Brandon Martínez)
### Interfaz y validaciones del formulario
* resources/views/habilitacion_create.blade.php
   - **Vista de la funcionalidad 2**: esta contiene el html, layaout principal obtenido desde **"resources/views/layouts/app.blade.php"**, ademas de campos dinamicos, barra de navegacion obtenido desde **"resources/views/layouts/navigation.blade.php"** modal de confirmacion obtenido desde **"resources/views/components/modal.blade.php"** y validaciones, de la **linea 1 - 360**, tambien se utilizan recursos adicionales dentro del formulario principal, ademas los estilos los obtiene desde **"public/css/form.css"**.

   - **Validacion.js** : Este se encuentra ubicado **"public/js/validacion.js"**, valida los campos del formulario antes de enviarlos, hace una llamada ajax para verificar los profesores con el metdo **"checkLimit"**, muestra errores sin recargar la pagina, y previene envio de formularios invalido esto es de **"la linea 1-163"**
   
   - **formHabilitacion.js**: Muestra/oculta secciones del formulario segun el tipo de habilitacion seleccionado, actualiza el UI (interfaz de usuario) sin recargar la pagina **"la linea 1-36"**

### Logica del backend
* app\Http\Controllers\HabilitacionController.php
   - **Metodo "create"**: **Dentro de esto de la linea 59 - 75** crea la logica de la primera parte del formulario, llamando a los alumnos que no tengan habilitacion y los semestres disponibles para la habilitacion.

   - **Metodo "Store"**:  **Ubicado en las lineas 113 - 177** Esta funcion valida los datos utilizando "StoreHabilitacionRequest" ubicado en \app\Http\Requests , ademas se utilizan validaciones extras para que los profesores no se repitan en una misma habilitacion en distintos roles con la funcion **"validateMultipleRoles"** que esta modularizado desde la **linea 297 - 309** y la validaciones para que los profesores puedan tener como maximo 5 habilitaciones por semestres con la funcion **"checkProfessorLimits" ubicado de la linea 314 - 343**, ademas crea la habilitacion en la base de datos y despliega los mensajes correspondientes.

   - **Metodo checkLimit**: en un endpoint AJAX que valida si los profesores que se seleccionaron en el formulario supera el limite, esta se ejecuta antes de enviar el formulario y da feedback al usuario de inmediato este esta ubicado de **la linea 348 - 376**.
